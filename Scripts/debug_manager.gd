...
# ========================================
# Hotkeys (ปรับได้ตามต้องการ)
# ========================================
const TOGGLE_DEBUG_KEY = KEY_F1      # เปิด/ปิด Debug Mode
const TOGGLE_MENU_KEY = KEY_F2       # เปิด/ปิด Debug Menu
const NEXT_DAY_KEY = KEY_F3          # ข้ามวัน
const SKIP_DIALOGUE_KEY = KEY_F4     # ข้าม Dialogue
const SKIP_QTE_KEY = KEY_F5          # ข้าม QTE
const SPEED_UP_KEY = KEY_F6          # เพิ่มความเร็ว Player
const SLOW_DOWN_KEY = KEY_F7         # ลดความเร็ว Player
const RESET_SPEED_KEY = KEY_F8       # รีเซ็ตความเร็ว
const COMPLETE_ALL_QUESTS_KEY = KEY_F9  # ทำ Quest ทั้งหมดให้สำเร็จ
const GOD_MODE_KEY = KEY_F10         # God Mode (ไม่ติด QTE)

func _ready():
	layer = 999  # อยู่ด้านบนสุด
	_create_debug_ui()
	_create_debug_menu()
	_update_visibility()
...
func _input(event):
	...
	match event.keycode:
		...
		SKIP_DIALOGUE_KEY:
			if debug_enabled:
				_debug_skip_dialogue()
		SKIP_QTE_KEY:
			if debug_enabled:
				_debug_skip_qte()
		SPEED_UP_KEY:
			if debug_enabled:
				_debug_speed_up_player()
		SLOW_DOWN_KEY:
			if debug_enabled:
				_debug_slow_down_player()
		RESET_SPEED_KEY:
			if debug_enabled:
				_debug_reset_player_speed()
		COMPLETE_ALL_QUESTS_KEY:
			if debug_enabled:
...
func _create_debug_menu():
	"""สร้าง Debug Menu ตรงกลางจอ"""
	debug_menu = Panel.new()
	debug_menu.name = "DebugMenu"
	debug_menu.custom_minimum_size = Vector2(500, 600)
	debug_menu.process_mode = Node.PROCESS_MODE_ALWAYS  # ⭐ ทำงานแม้เกม Pause
	...
	scroll.vertical_scroll_mode = ScrollContainer.SCROLL_MODE_SHOW_ALWAYS  # ⭐ แสดง Scrollbar เสมอ
	debug_menu.add_child(scroll)
	
	var vbox = VBoxContainer.new()
	vbox.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	vbox.add_theme_constant_override("separation", 5)  # ⭐ เพิ่มระยะห่าง
	...
	var god_btn = _add_toggle(vbox, "God Mode (F10): ", god_mode, _debug_toggle_god_mode)
	...
	var fps_toggle = _add_toggle(vbox, "Show FPS: ", show_fps, func(val): show_fps = val)
	var pos_toggle = _add_toggle(vbox, "Show Position: ", show_position, func(val): show_position = val)
...
func toggle_debug_mode():
	...
	print("[DEBUG] Debug Mode: %s" % ("ON" if debug_enabled else "OFF"))

func toggle_debug_menu():
	is_menu_open = not is_menu_open
	debug_menu.visible = is_menu_open
	
	# ⭐ ไม่ Pause เกม - ให้เล่นได้ต่อ
	# แค่แสดงเมนูทับอยู่
	print("[DEBUG] Menu: %s" % ("OPEN" if is_menu_open else "CLOSED"))
...
func _debug_next_day():
	print("[DEBUG] Forcing next day...")
	if has_node("/root/DayManager"):
		# Force complete all quests
		_debug_complete_all_quests()
...
func _debug_jump_to_day(day: int):
	print("[DEBUG] Jumping to day %d..." % day)
	if has_node("/root/DayManager"):
...
# ========================================
# Debug Functions - Quest
# ========================================

func _debug_complete_all_quests():
	print("[DEBUG] Completing all quests for today...")
	...
	for quest_data in quests:
		if not quest_data.done:
			...
			print("[DEBUG] ✓ Completed: %s" % quest_data.name)

func _debug_reset_all_quests():
	print("[DEBUG] Resetting all quests for today...")
	if not has_node("/root/QuestManager") or not has_node("/root/DayManager"):
		return
	...
	for quest_data in quests:
		if quest_data.id in QuestManager.quests:
			...
			print("[DEBUG] ✓ Reset: %s" % quest_data.name)

func _debug_complete_current_quest():
	# หา Quest Area ที่ใกล้ที่สุด
	var player = get_tree().get_first_node_in_group("player")
	if not player:
		return
	...
	if nearest_quest and nearest_quest.has_method("get_quest_id"):
		...
		print("[DEBUG] ✓ Completed nearest quest: %s" % quest_id)

# ========================================
# Debug Functions - Dialogue
# ========================================
...
func _debug_force_close_dialogue():
	print("[DEBUG] Force closing dialogue...")
	if has_node("/root/SystemDialogueManager"):
		SystemDialogueManager.hide_dialogue()
...
func _debug_toggle_god_mode():
	...
	print("[DEBUG] God Mode: %s" % ("ON" if god_mode else "OFF"))
	
	# Auto-complete any active QTE
	if god_mode and has_node("/root/QTEManager") and QTEManager.is_active():
		_debug_auto_complete_qte()

# ========================================
# Debug Functions - Player
# ========================================

func _debug_speed_up_player():
	player_speed_multiplier *= 2.0
	_apply_player_speed()
	print("[DEBUG] Player speed: x%.1f" % player_speed_multiplier)
...
func _debug_reset_player_speed():
	...
	_apply_player_speed()
	print("[DEBUG] Player speed: x1.0")

func _apply_player_speed():
	var player = get_tree().get_first_node_in_group("player")
	if player and "speed" in player:
		# Reset to base speed first (assuming base speed is 120)
...
func _debug_teleport_to_charger():
	...
	if player and charger:
		...
		print("[DEBUG] Teleported to charger")
